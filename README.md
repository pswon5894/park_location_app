...
사용자에 연령에 따라 버튼이 커지거나, 버튼 수를 줄이거나 기능의 작업절차를 줄이던가 복잡도를 조절해, ui와 ux를 조절해야한다

앱은 웹에 비해서 업데이트 절차가 많이서 시간이 걸린다, 앱 절차가 있음, 웹뷰 형식으로 바꾸어서 웹만 갈이끼는 방식으로 앱 업데이트를 줄일수 있음
...
gps 기반 위치 공유라면 지하주차장에서는 gps가 작동하지 않을 가능성이 있다
